...
def load_stare_dataset(base_path):
    # Find all images
    image_paths = sorted(glob.glob(os.path.join(base_path, '*.tif')))

    # Find manual annotations
    manual1_dir = os.path.join(base_path, 'manual1')

    # Pair each image with annotations
    valid_image_paths = []
    gt_paths = []
    ...
    for img_path in image_paths:
        ...
        # Construct path to corresponding ground truth image
        manual1_path = os.path.join(manual1_dir, img_name)
...
def main():
    ...
    stare_path = 'Fundus Databases/STARE Vessel Isolation'
    ...
    if os.path.exists(stare_path):
        # Load dataset
        image_paths, gt_paths = load_stare_dataset(stare_path)
        ...
        if image_paths:
            ...
            with open('STARE Performance.md', 'w') as f:
                f.write(readme_content)

            print("README file 'STARE Performance.md' has been generated.")
        else:
